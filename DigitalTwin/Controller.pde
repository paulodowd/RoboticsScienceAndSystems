...
class Controller_c {
  ...
  final float LINE_THRESHOLD = 1600;
  // Keep these values aligned with StampC3_Template/Controller.h.
  final float BASE_PWM = 40;
  final float LINE_FOLLOW_GAIN = 0.005;
  final float RECOVERY_LEFT_PWM = -55;
  final float RECOVERY_RIGHT_PWM = 55;
  ...
  void reset() { signal = 0; mode = WAITING; lastUpdateMs = lastTelemetryMs = recoveryStartMs = 0; }
  ...
  void setSignal(int requested) {
    if (requested == 0) { reset(); return; }
    if (signal == 0) { signal = 1; mode = FOLLOWING; recoveryStartMs = 0; }
  }
  /**
  ...
  void update(RobotSimulation_c robot) {
    ...
    lastUpdateMs = now;
    robot.getSurfaceSensors(); robot.getEncoders(); robot.getPose();
    if (signal == 1) runLineFollower(robot, now); else robot.setMotorPWM(0, 0);
    if (now - lastTelemetryMs >= TELEMETRY_INTERVAL_MS) { lastTelemetryMs = now; publishTelemetry(robot); }
  }
  ...
  void runLineFollower(RobotSimulation_c robot, long now) {
    ...
    if (!detected && mode == FOLLOWING) { mode = SEARCHING; recoveryStartMs = now; }
    if (mode == SEARCHING && detected) mode = FOLLOWING;
    if (mode == SEARCHING && now - recoveryStartMs >= RECOVERY_TIMEOUT_MS) mode = STOPPED;
    if (mode == STOPPED) { robot.setMotorPWM(0, 0); return; }
    if (mode == SEARCHING) { robot.setMotorPWM(RECOVERY_LEFT_PWM, RECOVERY_RIGHT_PWM); return; }
